...
export default function App() {
  ...
  useEffect(() => { try { localStorage.setItem("ml-theme", theme); } catch {} }, [theme]);

  // League
  const [leagueId, setLeagueId] = useState(localStorage.getItem(LS_KEY + "-leagueId") || "lega-andrea-2025");
  ...
  useEffect(() => { try { if (clubMode) sessionStorage.setItem("ml-club-mode","1"); else sessionStorage.removeItem("ml-club-mode"); } catch {} }, [clubMode]);
  ...
  useEffect(() => { completeMagicLinkIfPresent().catch(() => {}); }, []);

  // clientId + mute snapshot
  const clientIdRef = useRef(null);
  if (!clientIdRef.current) {
    const saved = (() => { try { return localStorage.getItem("ml-client-id"); } catch { return null; } })();
    if (saved) clientIdRef.current = saved;
    else {
      const nid = Math.random().toString(36).slice(2, 10);
      clientIdRef.current = nid;
      try { localStorage.setItem("ml-client-id", nid); } catch {}
    }
  }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const valid = fromCloud && typeof fromCloud === "object" && Array.isArray(fromCloud.players) && Array.isArray(fromCloud.matches);
        if (valid) {
          ...
          if (!migrated.bookingConfig.pricing) migrated.bookingConfig.pricing = getDefaultBookingConfig().pricing;
          if (!migrated.bookingConfig.addons) migrated.bookingConfig.addons = getDefaultBookingConfig().addons;
          setState(migrated);
          ...
          try { await saveLeague(leagueId, { ...initial, _updatedAt: Date.now(), _rev: 1, _lastWriter: clientIdRef.current }); } catch {}
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    try {
      unsub = subscribeLeague(leagueId, (cloudState) => {
        const valid = cloudState && typeof cloudState === "object" && Array.isArray(cloudState.players) && Array.isArray(cloudState.matches);
        if (!valid) return;
        ...
        setState((prev) => {
          const localRev = prev?._rev ?? 0, cloudRev = migrated?._rev ?? 0;
          const localTs  = prev?._updatedAt ?? 0, cloudTs = migrated?._updatedAt ?? 0;
  ...
  useEffect(() => {
    ...
    const hasChanges = !localStorage.getItem(LS_KEY) || relevantFields.some(field => 
      JSON.stringify(state[field]) !== JSON.stringify(JSON.parse(localStorage.getItem(LS_KEY))[field])
    );
    ...
    if (hasChanges) {
      try { 
        ...
        const t = setTimeout(() => { saveLeague(leagueId, toSave).catch(() => {}); }, 800);
  ...
  useEffect(() => {
    if (!clubMode && new Set(["giocatori","crea","prenota","tornei"]).has(active)) setActive("classifica");
  }, [clubMode, active]);

  const derived = useMemo(() => (state ? recompute(state.players || [], state.matches || []) : { players: [], matches: [] }), [state]);
  const playersById = useMemo(() => Object.fromEntries((derived.players || []).map((p) => [p.id, p])), [derived]);
  ...
  if (authLoading) {
    return (
      <div className={`min-h-screen ${T.pageBg} ${T.text} flex items-center justify-center`} data-theme={theme}>
        ...
        <header className={`sticky top-0 z-20 ${T.headerBg}`}>
          <div className="max-w-6xl mx-auto px-3 sm:px-4 py-3 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              <img src={LOGO_URL} alt="Marsica Padel" className="h-8 w-auto rounded-md shadow shrink-0" />
              <div className="text-lg sm:text-2xl font-bold tracking-wide truncate">
                Marsica <span className="bg-gradient-to-r from-emerald-400 to-lime-400 bg-clip-text text-transparent">Padel</span> League
              </div>
            </div>
            <button type="button" aria-label="Cambia tema" onClick={() => setTheme((t) => (t === "dark" ? "light" : "dark"))} className={`ml-1 px-3 py-1.5 rounded-xl text-sm transition ring-1 ${T.ghostRing} shrink-0`} title={theme === "dark" ? "Passa al tema chiaro" : "Passa al tema scuro"}>
              {theme === "dark" ? "☀️" : "🌙"}
            </button>
          </div>
        </header>

        {theme === "dark" && <div className="pointer-events-none fixed inset-x-0 top-0 h-40 bg-gradient-to-b from-emerald-500/20 to-transparent blur-2xl" />}
        ...
        {!state ? (
          ...
          <>
            {active === "giocatori" && clubMode && (
              <Giocatori T={T} state={state} setState={setStateSafe} onOpenStats={openStats} playersById={playersById} />
            )}
            {active === "classifica" && (
              <Classifica T={T} players={derived.players} matches={derived.matches} onOpenStats={openStats} />
            )}
            {active === "crea" && clubMode && (
              <CreaPartita T={T} state={state} setState={setStateSafe} playersById={playersById} onShowFormula={setFormulaText} derivedMatches={derived.matches} />
            )}
            {active === "stats" && (
              <StatisticheGiocatore T={T} players={derived.players} matches={derived.matches} selectedPlayerId={selectedPlayerId} onSelectPlayer={setSelectedPlayerId} onShowFormula={setFormulaText} />
            )}
            {active === "prenota" && clubMode && (
              <PrenotazioneCampi T={T} state={state} setState={setStateSafe} players={derived.players} playersById={playersById} />
            )}
            ...
            {active === "extra" && (
              <Extra T={T} state={state} setState={setStateSafe} derived={derived} leagueId={leagueId} setLeagueId={setLeagueId} clubMode={clubMode} setClubMode={setClubMode} />
            )}
          </>
        )}
      </main>

      <Modal open={!!formulaText} onClose={() => setFormulaText("")} title="Formula calcolo punti (RPA) – Spiegazione" T={T}>
        {formulaText}
      </Modal>
    </div>
  );
