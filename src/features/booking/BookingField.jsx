...
function BookingField({ user, T, state, setState }) {
  ...
  useEffect(() => {
    ...
    const loadInitialBookings = async () => {
      try {
        ...
        console.error('Errore caricamento prenotazioni iniziali:', error);
      }
    };
    loadInitialBookings();
  }, [user]);
  ...
  useEffect(() => {
    ...
    const mergePublicAndApp = async () => {
      try {
        ...
        console.error('Errore merge prenotazioni pubbliche:', e);
      }
    };
    mergePublicAndApp();
  ...
  const newBooking = await createBooking(bookingData, user);
      ...
      // Aggiorna stato locale e App.state per riflettere subito in "Prenotazione Campi"
