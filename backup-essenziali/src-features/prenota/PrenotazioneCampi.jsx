...
export default function PrenotazioneCampi({ state, setState, players, playersById, T }) {
  ...
  const dayRates = useMemo(
    () => timeSlots.map((t) => getRateInfo(t, cfg, null).rate),
    [timeSlots, cfg]
  );
    ...
    if (!editingId) {
      ...
      setState((s) => ({
        ...
        bookings: (s.bookings || []).map((b) =>
          b.id === editingId ? { ...b, ...baseBooking, updatedAt: Date.now() } : b
        ),
      }));
    }
    setModalOpen(false);
  ...
  return (
    <Section title="Prenotazione Campi" T={T}>
      ...
      <div
        className={`flex flex-col sm:flex-row sm:items-end gap-4 mb-6 ${T.cardBg} ${T.border} p-4`}
      >
        <div className={`flex items-center gap-2 rounded-xl px-4 py-2 shadow-lg ${T.cardBg}`}>
          <button
            type="button"
            className={`${T.btnGhostSm} text-lg font-bold`}
            onClick={() => goOffset(-1)}
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            onChange={(e) => setDayFromInput(e.target.value)}
            className={T.input}
          />
        </div>
        <div className="grow" />
        <div className="flex flex-col items-end">
          <span className={`text-xs font-medium ${T.subtext}`}>
            ...
            • {cfg.dayStartHour}:00–{cfg.dayEndHour}:00
          </span>
        </div>
      </div>

      <div
        className={`text-2xl font-bold mb-6 text-center bg-gradient-to-r from-blue-500 to-purple-500 bg-clip-text text-transparent drop-shadow-lg dark:from-emerald-400 dark:to-lime-400`}
      >
        {dayLabel}
      </div>
          ...
          <div className={`rounded-2xl p-6 shadow-2xl ${T.cardBg} ${T.border} backdrop-blur-lg`}>
            <div className="grid sm:grid-cols-2 gap-4">
                ...
                {cfg.addons?.lightingEnabled && (
                  <label className="inline-flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setForm((f) => ({ ...f, useLighting: e.target.checked }))}
                    />
                ...
                {cfg.addons?.heatingEnabled && (
                  <label className="inline-flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setForm((f) => ({ ...f, useHeating: e.target.checked }))}
                    />
                ...
                {[
                  ...
                  <div className="flex flex-col gap-1" key={key}>
                    ...
                    <input
                      ...
                      className={T.input}
                      placeholder="Digita nome o scegli"
                    />
                  </div>
                ))}
              </div>
              ...
              <div className="sm:col-span-2 grid sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1">
                  <label className={`text-xs font-semibold ${T.subtext}`}>
                    Ospite 1 (nome libero)
                  </label>
                  <input
                    value={form.guest1}
                    onChange={(e) => setForm((f) => ({ ...f, guest1: e.target.value }))}
                    className={T.input}
                    placeholder="Es. Mario (ospite)"
                  />
                </div>
                <div className="flex flex-col gap-1">
                  <label className={`text-xs font-semibold ${T.subtext}`}>
                    Ospite 2 (nome libero)
                  </label>
                  <input
                    value={form.guest2}
                    onChange={(e) => setForm((f) => ({ ...f, guest2: e.target.value }))}
                    className={T.input}
                    placeholder="Es. Luca (ospite)"
                  />
                </div>
              </div>
              ...
              <div className="sm:col-span-2 flex gap-2 flex-wrap mt-2">
                <button type="button" onClick={saveBooking} className={T.btnPrimary}>
                  {editingId ? 'Aggiorna prenotazione' : 'Conferma prenotazione'}
                </button>
                <button type="button" onClick={() => setModalOpen(false)} className={T.btnGhost}>
                  Annulla
                </button>
                {editingId && (
                  <button
                    ...
                    className="bg-gradient-to-r from-rose-500 to-rose-600 text-white font-bold px-6 py-2 rounded-xl shadow-lg hover:scale-105 transition"
                  >
